  ...
  return (
    <div className="mt-4 grid gap-2 sm:grid-cols-3">
      <a
        ...
        className="rounded-xl bg-purple-400 px-3 py-2 text-center text-xs font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#6b5df0]"
      >
        Ver en OCC Mundial
      </a>
      <a
        ...
        target="_blank"
        rel="noreferrer noopener"
        className="rounded-xl bg-blue-400 px-3 py-2 text-center text-xs font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#4a97f2]"
      >
        Ver en LinkedIn
      </a>
      <a
        ...
        target="_blank"
        rel="noreferrer noopener"
        className="rounded-xl bg-orange-400 px-3 py-2 text-center text-xs font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#ef934b]"
      >
        Ver en Indeed
      </a>
    </div>
...
export function ResultsPage() {
  ...
  const careersWithBudgetAlert = results.careers.filter(
    (career) => career.exceedsBudget,
  );
  ...
  return (
    <AppShell>
              ...
              <Bar dataKey="rawScore" radius={[10, 10, 0, 0]}>
                {chartData.map((entry) => (
                  <Cell
                    key={entry.area}
                    fill={entry.color}
                    stroke={entry.isTop ? "#F97316" : "transparent"}
                    strokeWidth={entry.isTop ? 1 : 0}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      <section className="card mb-6 border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-700">
        ...
        <p className="mt-2">
          <strong>Percentiles (P):</strong> P70 significa que tu puntaje está en
          el percentil 70, es decir, superaste al 70% de los estudiantes
          evaluados en esta área.
        </p>
        <p className="mt-2">
          <strong>⚠️ Advertencia de Costo:</strong> Las carreras marcadas con
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {careersWithoutBudgetAlert.slice(0, 3).map((career) => (
            ...
            >
              ...
              <p className="text-sm text-slate-600">
                Costo mensual aprox.:{" "}
                {career.monthly_cost_approx
                  ? `$${career.monthly_cost_approx.toLocaleString("es-MX")}`
                  : "N/D"}
              </p>
